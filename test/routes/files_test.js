...
suite('/files', () => {
  ...
  suite('GET /files', () => {
    test('should succeed if file exists', async function() {
      let res = await get(3000, `/devices/${id}/files?filepath=/data/b2g/mozilla/profiles.ini`);
      res.statusCode.should.equal(200);
      res.body.length.should.be.gt(0);
    });

    test('should fail if not found', async function() {
      let res = await get(3000, `/devices/${id}/files?filepath=/data/b2g/mozilla/wat`);
      res.statusCode.should.equal(404);
    });

    test('should fail if no file specified', async function() {
      let res = await get(3000, `/devices/${id}/files`);
      res.statusCode.should.equal(422);
    });

    test('should fail if file blank', async function() {
      let res = await get(3000, `/devices/${id}/files?filepath=`);
      res.statusCode.should.equal(422);
    });
  });
  ...
  suite('PUT /files', () => {
    ...
    test('should upload OK', async function() {
      let url = `http://localhost:3000/devices/${id}/files?filepath=/data/b2g/mozilla/profiles.ini`;
      let res = await upload(url, uploadFile);
      res.statusCode.should.equal(200);
    });

    test('should fail if no filepath', async function() {
      let url = `http://localhost:3000/devices/${id}/files`;
      let res = await upload(url, uploadFile);
      res.statusCode.should.equal(422);
    });

    test('should fail if filepath empty', async function() {
      let url = `http://localhost:3000/devices/${id}/files?filepath=`;
      let res = await upload(url, uploadFile);
      res.statusCode.should.equal(422);
    });
