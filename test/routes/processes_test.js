...
suite('/processes', () => {
  ...
  suite('GET /processes', () => {
    test('should get many', async function() {
      let res = await request('GET', 3000, `/devices/${id}/processes`);
    ...
    test('should get single', async function() {
      let res = await request('GET', 3000, `/devices/${id}/processes/4669`);
  ...
  suite('DELETE /processes/:pid', () => {
    test('should succeed', async function () {
      let res = await request('DELETE', 3000, `/devices/${id}/processes/1234`);
      res.statusCode.should.equal(200);
    });
  });
